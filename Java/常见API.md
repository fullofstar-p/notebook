...
public class User implements Cloneable {
    ...
    protected Object clone() throws CloneNotSupportedException {
        ...
        /*clone方法是由protected修饰的,他只能在本包和其他包的子类中使用,
          所以在测试类中不能直接使用User对象直接调用clone方法方法,由于User类是继承于Object类的
          要在User类中重写clone方法,然后在测试类中才可以使用User对象来调用该方法.
        */
...
public class ObjectsDemo01 {
    ...
    public static void method_02() {
        ...
        //3.此时s1是student类型,所以最终还是调用student中的equal方法
...
趋近于这个十进数数的二进制数据； 这样使用一个不太准确的数据进行运算的时候， 最终就会造成精度丢失；为了提高精度，Java就给我们提供了BigDecimal供我们进行数据运算。
