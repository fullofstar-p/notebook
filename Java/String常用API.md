...
- 直接赋值方式创建
  ...
  ​	以“”方式给出的字符串，只要字符序列相同(顺序和大小写)，无论在程序代码中出现几次，JVM 都只会建立一个 String 对象，存在复用,不存在创建新的,并在字符串池中维护
...
4. 字符串拼接的底层原理
   ...
   **等号的右边有变量**:
   ...
   ​	在拼接的时候有变量.JDK8之前,底层会使用StringBuilder进行拼接,拼接后,再调用其toString方法转换为String类型,而toString方法的底层是直接new了一个字符串对象

   ​	JDK8之后,底层会预估字符串拼之后的总大小,把要拼接的内容都放再数组中,此时也是产生一个新的字符串

   ​	![](images/jdk8之前有变量字符串拼接.png)

   **结论**:

   如果有很多字符串变量拼接,不要直接+,在底层会创建多个对象,浪费时间,浪费性能
